...
function processEncryptedData(decryptedData) {
    ...
    // Check if the cached data is outdated (missing the new InstPrem/Dues/FUP fields)
    if (excelData && excelData.length > 0 && excelData[0].policies && excelData[0].policies.length > 0) {
        if (excelData[0].policies[0].instPrem === undefined || excelData[0].policies[0].fup === undefined) {
            alert("System Update: We added a new FUP column! Please upload your DETAILS.xlsx file again so the system can extract these new values.");
